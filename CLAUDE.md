...
14. **Evaluación de Sorteos delegada a la DB (PostgreSQL PL/pgSQL)** — La lógica transaccional contable y de premios de sorteos se ejecuta nativamente en Postgres mediante `fn_evaluate_sorteo` y `fn_revert_sorteo` para evitar bloqueos del Event Loop de Node.js y picos de consumo.
15. **Sincronización Híbrida de Statements (Node.js background)** — La sincronización de `AccountStatement` se ejecuta exclusivamente en la capa de servicios de Node.js (`syncSorteoStatements`) de forma asíncrona, recalculando los balances del día completo mediante filtros por `businessDate`. Esto garantiza aislamiento multitenant y consistencia de saldos diarios e históricos.
16. **Proyecciones de Prisma en Listado de Tickets** — En `src/repositories/ticket.repository.ts`, el listado de tickets (`GET /api/v1/tickets`) debe utilizar proyecciones de campos (`select`) en lugar de devolver objetos completos, evitando la hidratación de datos pesados en memoria que causan caídas del heap.
17. **Sockets TCP y Timeouts para Reverse Proxy (Render)** — En `src/server/server.ts`, `server.keepAliveTimeout` debe fijarse en 65s (mayor a los 60s de Render) y `server.headersTimeout` en 66s con `backlog: 511` en `listen()` para evitar race conditions TCP (502/503) y soportar ráfagas concurrentes. Ver `docs/architecture/ESTANDAR_GESTION_CONEXIONES_HTTP_TCP.md`.
