...
// ADMIN-only
router.post('/', protect, restrictTo(Role.ADMIN), validateBody(createUserSchema), UserController.create);
router.patch('/:id', protect, restrictTo(Role.ADMIN), validateParams(idParamSchema), validateBody(updateUserSchema), UserController.update);
router.delete('/:id', protect, restrictTo(Role.ADMIN), validateParams(idParamSchema), UserController.remove);
