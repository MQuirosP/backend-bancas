...
async function main() {
  ...
  const jugadaMultiplierIds = jugadas.map((j: any) => j.multiplierId);
  const allMultipliers = await prisma.loteriaMultiplier.findMany();
  console.log('Todos los Multiplicadores en BD:', allMultipliers.filter(m => jugadaMultiplierIds.includes(m.id)));
}
